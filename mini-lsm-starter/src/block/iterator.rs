...
impl BlockIterator {
    ...
    fn seek_to(&mut self, index: usize) {
        ...
        let mut offset_slice = &self.block.data[offset..];
        let key_len = offset_slice.get_u16() as usize;
        let key = &offset_slice[..key_len];

        offset_slice.advance(key_len);
        let val_len = offset_slice.get_u16() as usize;
        let val_offset_start = offset + U16_SIZE + key_len + U16_SIZE;
